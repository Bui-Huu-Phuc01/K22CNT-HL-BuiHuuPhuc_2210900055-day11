...
// Component hiển thị trạng thái màu
const Badge = ({ text }) => {
    const color =
        text === "Đang học"
            ? "#22c55e"
...
// Cảnh báo GPA thấp / nợ tín chỉ
const Warn = ({ gpa, no }) => {
    if (Number(gpa) < 2)
        return <span title="GPA thấp" style={{ color: "#ef4444" }}>●</span>;
    if (Number(no) > 0)
        return <span title="Nợ tín chỉ" style={{ color: "#f59e0b" }}>●</span>;
    return null;
};

// Header có click sắp xếp
const HeadCell = ({ label, sortKey, sort, setSort }) => (
    <th
        onClick={() =>
            setSort({
                key: sortKey,
...
export default function BhpStudentList() {
    ...
    const remove = (id) => {
        if (window.confirm("Xoá sinh viên này?")) deleteStudent(id);
    };
        ...
        <div>
            ...
            <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                        <tr>
                            <HeadCell label="Mã SV" sortKey="maSV" sort={sort} setSort={setSort} />
                            <HeadCell label="Họ tên" sortKey="hoTen" sort={sort} setSort={setSort} />
                            <HeadCell label="Lớp" sortKey="lop" sort={sort} setSort={setSort} />
                            <HeadCell label="Ngành" sortKey="nganh" sort={sort} setSort={setSort} />
                            <HeadCell label="GPA" sortKey="gpa" sort={sort} setSort={setSort} />
                            <th>Tín chỉ Đ/N</th>
                            <HeadCell label="Trạng thái" sortKey="trangThai" sort={sort} setSort={setSort} />
                            <th>Tags</th>
                    ...
                    <tbody>
                        {list.map((sv) => (
                            <tr key={sv.id} style={{ borderTop: "1px solid #eee" }}>
                                ...
                                <td><Badge text={sv.trangThai || "Không rõ"} /></td>
                                <td>{(sv.tags || []).join(", ")}</td>
